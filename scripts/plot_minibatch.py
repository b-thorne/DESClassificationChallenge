...
parser.add_argument('--mode', choices=['raw', 'inference'],
                    help='Set mode to training or inferece')
...
def main():
    ...
    val_length = 100
    batch_size = 5
    trn, _, _ = load_and_split_dataset(ARGS.data_dir, ARGS.labels_path,
                                       trn_length, tst_length, val_length,
                                       batch_size)

    fig = plot_batch(next(enumerate(trn))[1])
    fig.savefig(f"plots/thumbnails_nbatch{batch_size}.png",
                bbox_inches="tight")
    return
